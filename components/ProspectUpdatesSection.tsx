...
export type ProspectUpdateEntry = {
  id: string;
  content: string;
  created_at: string;
...
export function ProspectUpdatesSection({
  prospectId,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error: insertError } = await supabase.from("prospect_updates").insert({
    ...
    await supabase.from("prospects").update({ latest_update: trimmed }).eq("id", prospectId);

    if (user) {
      await logSalesActivity(supabase, {
        actorId: user.id,
        actionType: "prospect_update_added",
        entityType: "prospect",
        entityId: prospectId,
        entityLabel: prospectLabel ?? null,
        summary: `Added progress update on prospect${prospectLabel ? ` “${prospectLabel}”` : ""}`,
        details: clipText(trimmed),
      });
    }
    ...
    <div className="card-elevated overflow-hidden">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Sales activity log for this pre-quote opportunity.
            </p>
          </div>
      ...
      <div className="border-b border-slate-100 bg-white p-4 sm:p-6">
        <form onSubmit={handleSubmit} className="space-y-3">
          <label htmlFor="prospect-update-content" className="flex items-center gap-2 text-sm font-medium text-slate-700">
            <MessageSquarePlus className="h-4 w-4 text-cyan-600" />
            Add progress update
          </label>
          <textarea
            id="prospect-update-content"
            value={content}
            onChange={(e) => setContent(e.target.value)}
            className="input-field min-h-[100px] resize-y"
            placeholder="Meeting notes, follow-ups, next steps…"
            rows={4}
            disabled={loading}
          />
          {error && (
      ...
      <div className="p-4 sm:p-6">
        {sorted.length === 0 ? (
          <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50/60 px-6 py-10 text-center">
            ...
            <p className="mt-3 text-sm font-medium text-slate-700">No updates yet</p>
            <p className="mt-1 text-xs text-slate-500">
              Record sales activity tied to this prospect so progress stays visible.
            </p>
          </div>
        ) : (
              ...
              return (
                <li key={update.id} className="relative flex gap-4 pb-8 last:pb-0">
                  ...
                  <article className="min-w-0 flex-1 rounded-xl border border-slate-200/80 bg-white p-4 shadow-sm">
                    ...
                    {isInitial &&
                      Math.abs(
                        new Date(update.created_at).getTime() - new Date(prospectCreatedAt).getTime()
                      ) < 120000 && (
                        <p className="mt-2 text-xs text-slate-400">Recorded at prospect creation</p>
                      )}
                  </article>
                </li>
              );

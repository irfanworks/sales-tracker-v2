...
export type PipelineUpdateEntry = {
  id: string;
  content: string;
  created_at: string;
...
export function PipelineUpdatesSection({
  projectId,
  ...
  projectId: string;
  updates: PipelineUpdateEntry[];
  projectCreatedAt: string;
  /** e.g. "EI-26001-00 · Project Name" for activity log */
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error: insertError } = await supabase.from("pipeline_updates").insert({
    ...
    await supabase.from("pipelines").update({ weekly_update: trimmed }).eq("id", projectId);

    if (user) {
      await logSalesActivity(supabase, {
        actorId: user.id,
        actionType: "pipeline_update_added",
        entityType: "pipeline",
        entityId: projectId,
        entityLabel: pipelineLabel ?? null,
        summary: `Added progress update on pipeline${pipelineLabel ? ` ${pipelineLabel}` : ""}`,

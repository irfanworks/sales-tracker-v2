-- Complete Project → Pipeline rename + fix aggregation RPCs.
-- Idempotent: safe whether 024 fully rolled back or partially applied.

-- ─── 0) Ensure pipelines table exists (rename from projects if needed) ───────
DO $$
BEGIN
  IF EXISTS (
    ...
    ALTER TABLE public.projects RENAME TO pipelines;
  END IF;
END $$;
...
BEGIN
  IF EXISTS (
    ...
    SELECT 1 FROM information_schema.tables
    WHERE table_schema = 'public' AND table_name = 'pipeline_updates'
  ) THEN
    ALTER TABLE public.project_updates RENAME TO pipeline_updates;
  END IF;
END $$;
...
-- Guard: pipelines must exist after this point
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.tables
    ...
    RAISE EXCEPTION 'public.pipelines still missing. Expected public.projects to exist for rename.';
  END IF;
END $$;
...
-- Aggregation RPCs: DROP first (cannot change OUT params with CREATE OR REPLACE)
